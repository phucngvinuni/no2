...
import utils # Assuming calc_psnr, calc_ssim are here
...
def evaluate_semcom_with_yolo(
    ...
    viz_output_dir: str,
    print_freq=20, # Adjusted default for less frequent eval prints
    visualize_batches=1, # Reduce default visualization
    visualize_images_per_batch=1 # Reduce default visualization
):
    semcom_net.eval()
    ...
    for batch_idx, (semcom_data_input, targets_tuple) in enumerate(dataloader):
        ...
        semcom_outputs_dict = semcom_net(
            img=original_imgs_tensor,
            bm_pos=bm_pos,
            _eval=True,
        ...
        rec_loss_val = reconstruction_criterion(reconstructed_image_batch, semcom_recon_target_gt)
        ...
        if 'vq_loss' in semcom_outputs_dict and semcom_outputs_dict['vq_loss'] is not None:
            ...
            rec_loss_val += vq_loss_tensor
            current_eval_vq_loss = vq_loss_tensor.item()
        rec_loss_meter.update(rec_loss_val.item() - current_eval_vq_loss, original_imgs_tensor.size(0))
        ...
        if yolo_model and map_calculator:
            if batch_idx < visualize_batches and viz_output_dir and CV2_AVAILABLE:
                # print(f"  Batch {batch_idx} - Running YOLO on ORIGINAL images (sanity check)...") # Optional
                try:
                    yolo_results_orig = yolo_model(original_imgs_tensor.detach(), verbose=False, conf=args.yolo_conf_thres, iou=args.yolo_iou_thres)
                    for i in range(min(len(yolo_results_orig), visualize_images_per_batch)):
                        res_orig = yolo_results_orig[i]
                        if hasattr(res_orig, 'plot'):
                            annotated_orig = res_orig.plot()
                            cv2.imwrite(os.path.join(viz_output_dir, f"ep{current_epoch_num}_b{batch_idx}_i{i}_YOLO_ON_ORIG.png"), annotated_orig)
                except Exception as e_yolo_orig: print(f"    Error running/plotting YOLO on original images: {e_yolo_orig}")

            try:
                yolo_results_list_recon = yolo_model(reconstructed_image_batch.detach(), verbose=False, conf=args.yolo_conf_thres, iou=args.yolo_iou_thres)
            except Exception as e_yolo_recon: yolo_results_list_recon = []; print(f"    Error running YOLO on reconstructed images: {e_yolo_recon}")

            yolo_preds_for_metric = []
            for i in range(len(yolo_results_list_recon)):
                result_recon = yolo_results_list_recon[i]
                if result_recon.boxes is not None:
                    yolo_preds_for_metric.append({
                        "boxes": result_recon.boxes.xyxy.to(device), "scores": result_recon.boxes.conf.to(device),
                        "labels": result_recon.boxes.cls.to(torch.int64).to(device)
                    })
                    if batch_idx < visualize_batches and i < visualize_images_per_batch and viz_output_dir and CV2_AVAILABLE:
                        if hasattr(result_recon, 'plot'):
                            annotated_frame_recon = result_recon.plot()
                            cv2.imwrite(os.path.join(viz_output_dir, f"ep{current_epoch_num}_b{batch_idx}_i{i}_snr{args.snr_db_eval:.0f}dB_YOLO_ON_RECON.png"), annotated_frame_recon)
                else: # Append empty if no boxes
                    yolo_preds_for_metric.append({"boxes": torch.empty((0,4), device=device), "scores": torch.empty(0, device=device), "labels": torch.empty(0, dtype=torch.int64, device=device)})
            
            if batch_idx < visualize_batches: # Optional print for debugging pred/gt formats
                # print(f"  Batch {batch_idx} - YOLO Preds for Metric (on RECON): {len(yolo_preds_for_metric)} sets")
                # print(f"  Batch {batch_idx} - YOLO GT for Metric: {len(yolo_gt_for_metric_device)} sets")
                pass
        ...
        if (batch_idx + 1) % print_freq == 0 or (batch_idx + 1) == len(dataloader):
            print(f'Test Batch {batch_idx+1}/{len(dataloader)}: '
                  f'[RecLoss: {rec_loss_meter.avg:.4f}] [VQLoss: {vq_loss_meter_eval.avg:.4f}] '
    ...
    if map_calculator:
        try:
            ...
            for k, v_tensor in final_map_results.items():
                v_item = v_tensor.item() if isinstance(v_tensor, torch.Tensor) else v_tensor
                print(f"  {k}: {v_item:.4f}")
        ...
        except Exception as e_map_compute: print(f"Error computing final mAP: {e_map_compute}"); final_stats['map'] = 0.0
...
def train_semcom_reconstruction_batch(
    ...
    if 'vq_loss' in outputs_dict and outputs_dict['vq_loss'] is not None:
        ...
        if isinstance(vq_loss_tensor, torch.Tensor): loss += vq_loss_tensor; current_vq_loss_val = vq_loss_tensor.item()
            
    return loss, reconstructed_image, current_vq_loss_val
...
def train_epoch_semcom_reconstruction(
    ...
    loss_scaler: torch.cuda.amp.GradScaler, args, max_norm: float = None, # Changed default for max_norm
    ...
    # print(f"\n--- Starting Training Epoch {epoch} ---") # Already printed in run_class_main
    ...
    for data_iter_step, (semcom_data_input, targets_tuple) in enumerate(data_loader):
        ...
        (original_images_for_loss, _) = targets_tuple
        original_images, original_images_for_loss = original_images.to(device, non_blocking=True), original_images_for_loss.to(device, non_blocking=True)
        ...
        with torch.amp.autocast(device_type=args.device, enabled=(loss_scaler is not None)):
            loss, reconstructed_batch, current_vq_loss_val = train_semcom_reconstruction_batch(
                model, samples_for_semcom_input, original_images_for_loss, bm_pos, criterion, args
            )
        ...
        clip_grad_val = max_norm if max_norm is not None and max_norm > 0 else None

        if loss_scaler is not None:
            loss_scaler(loss_for_backward, optimizer, parameters=model.parameters(), clip_grad=clip_grad_val, update_grad=((data_iter_step + 1) % update_freq == 0))
        else:
            loss_for_backward.backward()
            if (data_iter_step + 1) % update_freq == 0:
                if clip_grad_val: torch.nn.utils.clip_grad_norm_(model.parameters(), clip_grad_val)
